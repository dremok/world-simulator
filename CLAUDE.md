...
- **Next action:** (loop running, iter 15: /briefing endpoint + HUD card shipped; all pair storylines narrated) Next: narrate pass for storylines with fresh activity, entity tables, time slider over country_state, storyline clustering v2 (actor-aware keys). /diff count verified correct (day-one artifact only).
