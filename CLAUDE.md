...
- **Next action:** (loop running, iter 11: 245 events enriched, Iran narration refreshed; NEXT: actor-aware storyline keys, US grab-bag at heat 203 is the motivating case) Next: narrate pass for storylines with fresh activity, entity tables, time slider over country_state, storyline clustering v2 (actor-aware keys). /diff count verified correct (day-one artifact only).
