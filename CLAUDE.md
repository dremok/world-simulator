...
- **Next action:** (loop running, iter 12: actor-pair storyline keys shipped and re-derived — IRN-USA/RUS-UKR/ISR-SAU are now first-class narratives; grab-bags remain for single-actor events by design) Next: narrate pass for storylines with fresh activity, entity tables, time slider over country_state, storyline clustering v2 (actor-aware keys). /diff count verified correct (day-one artifact only).
