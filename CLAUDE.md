...
- **Next action:** (loop running, iter 10: actors panel shipped; entity pipeline cron-verified) Next: narrate pass for storylines with fresh activity, entity tables, time slider over country_state, storyline clustering v2 (actor-aware keys). /diff count verified correct (day-one artifact only).
