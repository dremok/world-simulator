...
6. **World MCP server.** Expose `world_state(country)`, `storylines(topic)`, `what_changed(since)` as MCP tools over the same DB. Any Claude session on any machine can then ask "what's happening in Brazil right now". Cheap to build once the API exists.
